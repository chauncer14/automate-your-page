...
EXAMPLE_LIST_OF_CONCEPTS = [ ['Lesson 4: Introduction to Serious Programming', 'Unlike machines, which are typically physically designed to do a single task, computers are programmable to perform multiple tasks.   Computers can pretty much do anything as long as we can write the sequence of instructions telling them what to do.  We call these sequences of instructions programs. Everything from web browsers to games and mobile apps are simply a precise sequence of steps, that a person wrote using one of the various programming languages for the computer to follow and execute.'],
                             ...
                             ['Lesson 7: Control Flow & Loops: if or else and while', 'Control flows help us execute different code based on certain conditions. If or else are keywords that let us tell what we want to run in different situations. While loop lets us repeat a block of code many times until a certain condition is met. Code under loops is always indented.'],
                             ['Lesson 8: Debugging', 'Debugging in an important step of writing code. The last line of Python error messages are the best place to start, as it will tell you what line the problem occurs on.'],
                             ['Lesson 9: Structured Data: Lists & For Loops', 'Lists are one way that we structure data. Lists are helpful because we can then run through all the elements of a list using a for loop.'],
                             ['Lesson 10:How to Solve Problems', 'Problems can be very complicated, but become much more manageable once broken down in to single functions. The first step in solving large problems is to understand the problem. Next, figure out what are the inputs, and what are the outputs.']]
